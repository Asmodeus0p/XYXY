...
import com.jiyun.asmodeus.xyxy.model.biz.ITeacherAttention;
...
public class MasterGuanzhuListActivity extends BaselivstActivity  {
    ...
    public static void start(Activity activity, String name, int teacher_id) {
        Intent intent = new Intent(activity, MasterGuanzhuListActivity.class);
    ...
    private void initView(){
        ...
        masterguanzhulist_aty_title_tv.setText(String.format("%s的关注",name));
                ...
                switch (view.getId()){
                    ...
                    case R.id.masterguanzhu_listitem_img:
                        ...
                        if(datalist.get(position).getUserType()>=2){
                            MasterDetailActivity.start((Activity) context,datalist.get(position).getAttentionId());
                        }else{
                            PersonInfoActivity.start((Activity) context,datalist.get(position).getAttentionId());
                        }
    ...
    private void loadContent(){
        ...
        HttpHelp.baseHttpRequest(context, Constant.Root_url)
                .create(ITeacherAttention.class)
                .getTeacherAttention(teacherId,HttpHelp.getUserId(context),page)
